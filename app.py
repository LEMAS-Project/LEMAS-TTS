...
# TTS on CUDA when available (inside the @spaces.GPU worker on Spaces),
...
class UVR5:
    ...
    def denoise(self, audio_info):
        # On HF Spaces this runs in the main process, where CUDA must not be
        # initialized. Force UVR5 to CPU there. Locally we can still prefer
        # CUDA if available.
        if IS_SPACES:
            device = "cpu"
        else:
...
with gr.Blocks(title="LEMAS-TTS Inference") as app:
    ...
    examples = gr.Examples(
        examples=[
            ...
            " pretending this was totally planned.",
            ],
            ...
            " and with preparations currently underway.",
            ],
        ],
        inputs=[
            ref_text,
